...
低分辨率特征: M_low 个点, C_high 维
    ...
    │    → 直接用 Grid Pooling 保存的映射，无需搜索
    │
    ▼ ② 通道对齐 + Skip Connection (相加):
    │    feat_decoder = Linear + BN + GELU  (C_high → C_out)
    │    feat_skip    = Linear + BN + GELU  (C_skip → C_out)
    │    feat = feat_decoder + feat_skip
    │
    ▼ ③ Attention Block × 2 精炼特征
    │
    ▼
...
低分辨率特征: M_low 个点, C_high 维
    ...
    ▼ ① Scatter-back 上采样:
    │    feat_up = feat_low[pooling_inverse]
    │    → 每个高分辨率点直接取它所属 voxel 的特征（确定性映射，无需搜索）
    │
    ▼ ② 通道对齐 + Skip Connection (相加):
    │    feat_decoder = Linear(C_high → C_out) + BN + GELU  (处理 decoder 特征)
    │    feat_skip    = Linear(C_skip → C_out) + BN + GELU  (处理 encoder skip 特征)
    │    feat_combined = feat_decoder + feat_skip
    │
    ▼ ③ Serialized Attention Block × 2 (默认配置):
...
> **与常见点云 Decoder 的区别**：PointNet++ 等方法使用 KNN 距离倒数加权插值（$f_i = \sum_j w_j f_j / \sum_j w_j$, $w_j = 1/\|p_i - p_j\|^2$），需要最近邻搜索。PTV3 直接复用 Grid Pooling 的 `torch.unique` 映射，同一 voxel 内的所有点共享相同特征，计算量为 $O(N)$ 的索引操作，无搜索开销。
