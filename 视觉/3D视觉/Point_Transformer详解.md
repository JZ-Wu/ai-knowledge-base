...
#### 每一步上采样（SerializedUnpooling + Attention Blocks）

PTV3 的 Decoder 上采样**不使用 KNN 插值**，而是利用 Encoder 下采样（Grid Pooling）时保存的映射关系直接 scatter-back：

```python
# Encoder 的 Grid Pooling 中:
cluster = torch_scatter.voxel_grid(...)       # 每个点属于哪个 voxel
_, inverse = torch.unique(cluster, return_inverse=True)
point_dict["pooling_inverse"] = inverse       # 保存映射，供 Decoder 使用
point_dict["pooling_parent"] = point          # 保存高分辨率点云
```
...
低分辨率特征: M_low 个点, C_high 维
    ...
    ▼ ① Scatter-back (voxel 内复制):
    │    feat_up = feat_low[pooling_inverse]
    │    → 同一 voxel 内的所有点拿到完全相同的特征（纯索引操作）
    │    ⚠️ 此时同 voxel 的点无法区分！
    │
    ▼ ② Skip Connection 打破重复:
    │    feat_decoder = Linear(C_high → C_out) + BN + GELU
    │    feat_skip    = Linear(C_skip → C_out) + BN + GELU  ← Encoder 保存的逐点特征
    │    feat = feat_decoder + feat_skip
    │    → skip 特征来自下采样前，保留了逐点差异，加上后恢复点间区分度
    │
    ▼ ③ Serialized Attention Block × 2:
    │    与 Encoder 相同的 Block 结构 (CPE → Attn → FFN)
    │    → 让相邻点交换信息，进一步精炼每个点的特征
    │
    ▼
高分辨率特征: M_high 个点, C_out 维
```

> **为什么 Skip + Attention 缺一不可？** Scatter-back 是"voxel 内复制"，同 voxel 的点特征完全相同。**Skip Connection** 加回 Encoder 对应层的特征（下采样前保存的，保留了逐点细粒度差异），直接打破重复。**Attention Block** 在此基础上让点与点之间交换上下文信息，进一步精炼差异。
>
> **与常见点云 Decoder 的区别**：PointNet++ 等方法使用 KNN 距离倒数加权插值（$f_i = \sum_j w_j f_j / \sum_j w_j$, $w_j = 1/\|p_i - p_j\|^2$），需要最近邻搜索，且插值本身就能产生逐点不同的特征。PTV3 用 scatter-back 虽然更快（$O(N)$ 索引，无搜索），但依赖 Skip + Attention 来恢复逐点差异。
