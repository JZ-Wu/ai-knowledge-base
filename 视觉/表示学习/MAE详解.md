...
| 变体 | 扩展方向 | 关键改动 |
|------|---------|---------|
| **VideoMAE** | 视频 | 时空 tube masking，mask ratio 高达 90%（视频时间冗余更大） |
...
VideoMAE 就是你提到的"时空编码器"的基础。核心思想：

- 输入视频 → 切成 **时空 tube**（如 2×16×16）
- mask ratio 提升到 **90-95%**（因为视频帧间冗余极大）
- Encoder 只处理可见 tube → 效率极高
- 学到的时空表示对视频理解任务（动作识别、时序定位）非常有效
