...
@router.post("/compact")
async def compact(_request: CompactRequest):
    """手动压缩会话上下文（已弃用：-p 模式无持久 session）。"""
    raise HTTPException(
        status_code=400,
        detail="Compact is not supported: CLI -p mode does not maintain persistent sessions.",
    )
