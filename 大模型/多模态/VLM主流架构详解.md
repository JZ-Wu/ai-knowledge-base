...
**三个目标的训练细节：一个 batch，三次前向，一个 loss**

每个训练 batch 包含 N 对 (图像, 文本)。对于同一个 batch，Q-Former 做 **三次前向传播**，每次切换不同的注意力掩码：
...
├── 正样本: batch 中原本配对的 (图, 文)
├── 难负样本构造 ← 🔑 这里用到了 ITC 的 sim_matrix:
│   ├── 对每张图，在 sim_matrix 的对应行中找 相似度高但不是正确配对 的文本
│   └── 对每段文本，在 sim_matrix 的对应列中找 相似度高但不是正确配对 的图像
│   （相似度高 = 模型容易混淆 → 强迫模型学更细粒度的区分）
├── 32 个 query 输出各自过一个二分类头 → 平均 → 匹配/不匹配
└── 用 Binary Cross-Entropy loss
