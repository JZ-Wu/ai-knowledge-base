...
2025 年 VLM 领域的核心主题：**原生多模态从概念变为现实，MoE 成为标配，架构进一步收敛**。
...
```
在 Qwen2-VL 基础上的改进:
  - 动态分辨率 v2: 更灵活的分辨率处理策略
  - 视频理解增强: 更长视频、更好的时序建模
  - Agent 能力: 支持 GUI 操作、工具调用等 agentic 任务
  - 架构不变: 仍然是 ViT + MLP + Qwen2.5 (MoE)
  → 再次印证"数据和训练 > 架构变化"
```
...
```
在 InternVL 2.5 基础上的改进:
  - 进一步扩展多模态能力
  - 更强的推理和 agent 能力
  - 仍然基于 InternViT + Pixel Shuffle + MLP 架构
```
...
```
1. 架构已经收敛
   开源社区: ViT + MLP + LLM（全参数端到端训练）已成标准范式
   架构层面能做的优化空间很小，竞争焦点转向数据和训练

2. MoE 成为标配
   Qwen2.5-VL、LLaMA 4、Kimi-VL、DeepSeek-VL2 全部使用 MoE
   总参数大（几十到几百 B）但激活参数少（十几 B）→ 性价比高

3. 原生多模态从概念到落地
   2024: 只有 Gemini 在做
   2025: LLaMA 4、Chameleon 等开始跟进
   但开源社区受限于算力，主流仍是三件套

4. 能力边界扩展
   VLM 不再只是"看图回答"，开始具备:
   - Agent 能力: 操作 GUI、调用工具（Qwen2.5-VL）
   - 推理能力: Thinking/CoT 模式（Gemini 2.5）
...
| Qwen2.5-VL | ViT-bigG | MLP | Qwen2.5 (MoE) | 否（三件套） | 2025 |
...
2024: 全部解冻，端到端训练 (InternVL2, Qwen2-VL)
         ↓ 架构收敛，竞争转向数据和训练
2025: 两条路线并行
      ├── 开源主流: 三件套 + MoE + 更强数据 (Qwen2.5-VL, InternVL3)
      └── 大厂前沿: 原生多模态预训练 (Gemini 2.5, LLaMA 4)
         ↓ 下一步？
2026: 原生多模态开源化？统一图/文/音/视频/代码？
```
