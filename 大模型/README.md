# 大模型 (LLM)

大语言模型相关的全部知识。

## 子专题

| 专题 | 说明 |
|------|------|
| [基础理论](基础理论/README.md) | Transformer 架构、注意力机制、MoE、Tokenizer、位置编码、Scaling Laws |
| [主流模型](主流模型/README.md) | GPT、Claude、LLaMA、Gemini、Qwen、DeepSeek 等 |
| [训练与微调](训练与微调/README.md) | 预训练、SFT、RLHF/DPO/GRPO、LoRA/QLoRA、数据工程 |
| [RAG与知识增强](RAG与知识增强/README.md) | 检索增强生成、向量数据库、Embedding |
| [多模态](多模态/README.md) | VLM、图像生成、语音、视频 |
| [推理优化](推理优化/推理优化详解.md) | KV Cache、FlashAttention、量化、vLLM、投机解码 |
| [评估与对齐](评估与对齐/幻觉与评测详解.md) | 幻觉原因与缓解、主流 Benchmark、评测方法 |

---
[返回总目录](../README.md)
